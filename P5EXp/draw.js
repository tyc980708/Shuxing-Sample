...
class raindrop {
	...
	draw(){
		...
		var xmod = 0.7*mousexp*(1.1-Math.pow(this.z,3))/5 + this.x;
		if(xmod > document.body.clientWidth){
			xmod -= document.body.clientWidth;
		}
		var ymod = 0.7*mouseyp*(1.1-Math.pow(this.z,3))/5 + this.y;
		...
		rect(xmod , ymod, this.size, ysize);
	}
}
